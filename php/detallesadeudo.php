...
<input type="submit" value="Abonar">
  ...
  $total=0;
      ...
      $sql='select * from adeudoproveedoores where rfc like "%'.$clave.'%" group by foliocompra';
      $result=mysqli_query($conexion,$sql);
      if (!$result) {
          echo("Error description: " . mysqli_error($conexion));die;
       }
...
*/
$sql_qry='SELECT SUM(total) AS count FROM comprasproducto  where rfc like  "%'.$clave.'%" ';
